...
- **playerbots** (~60%) — In-process bots as real `Player` rows and an empty-socket `WorldSession`, brought in through `World::AddSession`. They only act by queueing real client packets. They walk the navmesh to stand beside questgivers. Nearby turn-in stays a 40-yard search. Finished log quests walk to the map marker (`creature_questender` / quest POI) and turn in before kills or new accepts. Incomplete gameobject objectives walk that objective’s map marker, stand beside the spawned object, and queue `CMSG_GAME_OBJ_USE`. Incomplete kill objectives stay a 150-yard select-and-melee search (map-marker walks for kills are later), then nearby accept. Runtime keys live in `modules/mod-playerbots.conf` next to the server.
