...
- **reaper-class** (~45%) — Conquest of Azeroth's Reaper as class 16, in `modules/mod-coa`. Class id, `MAX_CLASSES` 17, and a sixteenth entry in the dodge, parry, and diminishing tables copied from Death Knight; the client game tables have one fixed column per class so class 16 reads Death Knight's. Hotfix rows for the class, Runic Power, three specs plus the required initial spec, five races, and a class skill line. World rows for start positions, level stats from Demon Hunter, and no expansion gate. `.coa reaper make` and `.coa reaper unmake` create and remove one without the creation screen; unmake also runs from the server console. Playerbots refuse the class. Character creation works on the real client through two loose interface files and a two-byte memory patch, all in `evryLoader`. Per-spec main stats: Harvest agility, Soul intellect, Domination strength. No spells, no starting kit, no talent trees, and the spec page still raises on unknown spec ids.
