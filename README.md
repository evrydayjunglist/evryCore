...
- **playerbots** (~80%) — In-process bots as real `Player` rows and an empty-socket `WorldSession`, brought in through `World::AddSession`. They only act by queueing real client packets. They walk the navmesh to stand beside questgivers. Nearby turn-in stays a 40-yard search. Incomplete log objectives (gameobject use, kill, item loot) run before walking to a finished-quest map marker. Incomplete gameobject objectives walk that objective’s map marker, stand beside the spawned object, and queue `CMSG_GAME_OBJ_USE`. Incomplete kill objectives walk that objective’s map marker, stand beside the spawned creature, and queue select and melee swing. Incomplete item objectives walk that objective’s map marker, kill the creature that drops the item, then queue `CMSG_LOOT_UNIT`, `CMSG_LOOT_ITEM`, and `CMSG_LOOT_RELEASE`. Nearby kills stay a 150-yard search. Then walk to the finished-quest ender, then nearby accept. Runtime keys live in `modules/mod-playerbots.conf` next to the server.
