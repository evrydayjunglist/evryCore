...
- **playerbots** (~60%) — Bots are real characters, logged in through an in-process session with no network connection, and they only act by queueing the packets a real client would send. Today they quest on their own: talk to quest givers they can see within 40 yards, fight what attacks them, loot corpses, use quest objects and quest items, and walk to the nearest objective marker, finished-quest `?`, or takeable `!` in their zone. In a fight they auto-attack and press the damaging spells they know, one press per global cooldown. They walk the navmesh but step on the real ground: no climbing steeper than 35°, no long drops, no walking through walls or objects, and they walk around a small lip or make one checked normal jump over it. When a route is refused and the small look around her feet cannot get her past it, she stops, maps the ground she can walk within 60 yards by those same rules, walks the best way round it finds toward her target, and asks the navmesh again from there. If nothing she can reach is closer, which is what being in a pocket looks like, she asks the navmesh for a route from the ways out of that ground and walks to the first one whose route she can actually start walking, even though it leads away from her target. For a bot that is stuck, a GM can type `.playerbots walkmap [yards] [name]` to get a picture page, written next to the server logs, of all the ground she can walk to from where she stands, which of it she cannot walk back from, and where and why each step out of it is refused. When the server roots, stuns, fears, knocks back, or teleports them, they answer the way a real client does: they stay put while held, fly a knockback's arc and land where it meets the floor (taking any fall damage), and send no movement until a teleport has finished. If their navmesh path keeps ending short of a target, they give that target up and look for other work instead of walking back and forth. A target they give up on is skipped only for a while: after two minutes with nothing to do they forget what they skipped and look again, and when the trouble is the spot they stand on (no step anywhere from it), the target is not skipped at all; they wait five seconds and look again. A quest giver hundreds of yards away is no longer dropped because the quick route check before a walk ran out of room; the walk's own long search decides. While a bot has nothing to do, the log says for each finished quest why its turn-in was not picked. On death they release, run back and take their corpse, and use the spirit healer only when a player would. When bags are full of junk or gear is nearly broken they sell and repair at a vendor with their own gold. `playerbots.exe` builds with the module and, with `Playerbots.LoginMode = Coordinator`, decides when bots are online; it does not make any gameplay decisions yet. A bot kicked by a GM or by AntiDOS logs back in after 30 seconds (in Coordinator mode, while `playerbots.exe` owns the roster); like a player, it stays offline while its account or character is banned, the world is closed, or the realm only admits higher security levels. The server can load a second set of movement maps built for what a player's body can walk instead of what a creature's can, from a `playermmaps` folder beside the usual one, and bots ask their routes of it on every map it covers; where it has not been generated they fall back to the creature set and nothing changes. The map generator takes `--maxClimb` for building that set; Kalimdor's has been generated. Each route a bot starts walking, and each route her recovery tests, is logged with a sentence saying what the navmesh search did: whether it reached the destination, ran out of room to search, found only the ground joined to where she stands, or found a route longer than a path can hold, and how long it took to build. Bots search their walks with room for a long route, in search objects of their own: up to 16,384 search steps and paths of up to 1,024 pieces, about 4,000 yards of road, where creatures keep the 1,024 steps and 74 pieces they have always had. Settings are in `modules/mod-playerbots.conf` next to the server (`Playerbots.Enable`, `Count`, `Races`, `Classes`). Still open: heals, buffs, pets, class rotations, and backing out of minimum range; and the external brain for long-term activity, groups, guilds, professions, and economy (see [Playerbots architecture](PLAYERBOTS_ARCHITECTURE.md)).
- **reaper-class** (~45%) — Conquest of Azeroth's Reaper as class 17, in `modules/mod-coa`. It was class 16 until 20 September, when it moved up one to leave 16 free for Hero, which the owner wants ahead of every Conquest of Azeroth class; class 16 is now a deliberate gap in the class enum. Class id, `MAX_CLASSES` 18, and a seventeenth entry in the dodge, parry, and diminishing tables copied from Death Knight; the client game tables have one fixed column per class so Reaper reads Death Knight's. Hotfix rows for the class, Runic Power, three specs plus the required initial spec, five races, and a class skill line. World rows for start positions, level stats from Demon Hunter, and no expansion gate. `.coa reaper make` and `.coa reaper unmake` create and remove one without the creation screen; unmake also runs from the server console. Playerbots refuse the class. Character creation works on the real client through two loose interface files and a two-byte memory patch, all in `evryLoader`. Per-spec main stats: Harvest agility, Soul intellect, Domination strength. Its four specialisations moved from the 9001 block to 610 to 613, because a specialisation id above the range the client ships makes every spell a Reaper knows draw as the wrong spell. No spells, no starting kit, no talent trees, and the spec page still raises, because the client's own Lua holds a hardcoded list of specialisation ids and no custom id is in it.
- **hero-class** (~40%) — Conquest of Azeroth's Hero as class 16, in its own module `modules/mod-hero` rather than beside Reaper, because Hero is a container for a classless character and the Free Pick economy that will fill it has nothing to do with Reaper. Class id, the stat tables, and its own column choice in the client game tables: Druid's, not Reaper's, because Death Knight's base mana is zero at every level and Hero shows a mana bar. Hotfix rows for the class, ten power types, one hidden specialisation, a class skill line, and every weapon proficiency. Selectable on the creation screen for all thirty-one races a Warrior can be, including Dracthyr, with start positions copied from each race's Warrior. Armour is every type including shields; specialisations are the one hidden row the server demands and nothing visible, which turns out to avoid the spec page fault Reaper hits. A Hero can learn another class's ability and use it: a borrowed Frostbolt cast, applied Chilled and hit for 52. Playerbots refuse the class. No spells of its own, no starting kit, and none of Free Pick or Wildcard exists yet, which is the point of the class.
