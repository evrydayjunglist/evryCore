...
- **battlepay** (~85%) — Classic Store glue and the free level-80 boost. Character-select packets use the 12.1.0 client layout with size checks, so the boost catalog no longer crashes the client.
- **timerunning** (~30%) — Configurable seasons in worldserver.conf with reload, a persistent season per character, separate seasonal world copies in the same realm, and the party, trade, mail, bank and map restrictions that keep seasonal and ordinary play apart. Pandaria creation waits on the client (build 12.1.0.69497 rejects season 1), and the reconstructed Timeless Isle opening is parked on `job/timerunning-pandaria-opening`. Full Remix progression, dungeons, rewards and conversion remain.
