...
- **archaeology** (~80%) — Survey, private finds, fragment loot, and client project-solve casts. Research* DB2 loads, sites persist with hidden find coordinates, and criteria types 3/4/6 plus modifiers 65/66 score. Site-to-branch is server policy (ResearchSite.db2 has no join). Four sites per seeded map (Eastern Kingdoms, Kalimdor, Outland, Northrend, Pandaria) when Archaeology is trained. Fragment crates, Chromie Time coupling, later continents, historical skill gates, and a sniffed find/keystone rate are still open.

### On `evry` only

- **rts-spike** (~95%) — See the [Commander Mode living plan and work tracker](COMMANDER_MODE.md). Phase 0A established the exact stock commentator camera, addon-channel round trip, camera-relative distant ground targeting, the usable spell 2000001 reticle, and vehicle-seat destinations. Phase 0B's bounded one-bot harness produced a live no-go for playable possession on the current stock-server path: Magey's camera and active mover switched to Opai, but selection and spell casts remained Magey's and Opai's stock known-spell/action packets did not replace Magey's visible UI. Explicit release restored Magey without relogging and resumed Opai's Builtin movement. The throwaway switch commands, actor-UI probe, playerbot-quiescing seam, and state tests were removed after the evidence was recorded; no production control seam was retained. Melee, action-bar persistence, death, and other lifecycle cases remain untested because the playable-possession gate already failed. The selected future boundary is now a capability-negotiated custom client/protocol comparable in responsibility to SuperUI. Client patching, injection, reverse engineering, custom opcodes, a purpose-built client, and companion tooling are available options; exact client form, protocol, safety proof, and unattended-original policy remain separate design work. World streaming around the commentator camera also remains unproven, spell 2000001 still needs separate product-data cleanup, and walking five bots at RTS scale remains Phase 1A's acceptance test.
- **rts-commandable-movement** (~70%) — The shared RTS controller now claims the group leader's original character and eligible grouped managed bots with one commander identity and per-subject generations. Move, redirect, persistent hold, release, lifecycle suspension, stale-request rejection, Coordinator presence pinning, and the packet walker are shared behind a subject-neutral API. Automated tests pass; live testing passed one-bot enter/move/stop/release and an owning-client-synchronized original-character move. The same run proved the stock client still sends accepted WASD movement for the original while RTS owns it, so exclusive original-character control has not passed. A narrow, version-pinned Commander client that parks direct body input, restores it on every exit, and negotiates capability with a server-side packet-origin gate is the next job. Five-bot scale, redirect, death, teleport, group/map invalidation, and Coordinator-loss acceptance remain open.
