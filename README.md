...
- **rts-spike** (~95%) — See the [Commander Mode living plan and work tracker](COMMANDER_MODE.md). Throwaway harness in `modules/mod-rts-spike/` that answers the questions about the retail client that source cannot: a scratch `.rtsspike` command family plus an `RTSSpike` addon. What it established. The commentator free camera works on an unmodified client: setting the two commentator player flags lets `C_Commentator.SetCameraPosition` move the camera to any point, exactly and unclamped, in the same world coordinates `.gps` prints, so no client patching and no vehicle seat are needed for a top-down view. The world does not stream around that camera, because visibility still follows the character through `m_seer`, so a commander view needs a real world object as the viewpoint the way farsight already works. The addon command channel round-trips on the stock `TrinityCore` prefix, in and out of camera mode. A ground-targeted cast delivers the clicked point to the server, and it follows the camera rather than the body: with the camera three hundred yards out, clicks under it arrived within about twelve yards of the camera while the character stayed put. The client, not the server, decides whether you may click at all, so an order spell needs a long `SpellRange` row and `SPELL_ATTR2_IGNORE_LINE_OF_SIGHT`. A fresh clone also needs its client-only `Spell.db2` root, not just `SpellName` and the server-loaded SpellInfo components; the repaired evryOps clone path now carries that root through `hotfix_blob`, and spell 2000001 passed learned, known, usable, reticle, `[@cursor]`, and vehicle-seated destination checks without a `SkillLineAbility` row. The temporary realm-wide override of Blizzard `SpellMisc` ID 164352 has been removed and its native record refreshed. The map for a clicked destination comes from the caster; the destination's own map is never filled in. Walking bots at RTS scale and commentator-camera composition with a server viewpoint are not answered. Delete the module once those remaining answers are recorded.
