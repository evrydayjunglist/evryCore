...
- Two core loot fixes found by the second playthrough. `Loot::hasItemFor` now counts an unlooted item that follows loot rules but carries database conditions. Its comment already said it covered conditional items, but only quest and free-for-all items were checked, so a corpse whose only loot was such an item never became lootable; the Thread of Power is one. And a consumable that takes effect when looted (`ITEM_SPELLTRIGGER_ON_LOOTED_FORCED`) and also carries a use effect without charges is no longer stored: taking it from a loot window, automatic storing and master loot all cast its looted spell on the looter instead. In the installed client data that is 55 items: the 36 Mists of Pandaria Remix threads, the 16 Legion Remix epoch mementos, Bronze Cluster, Infinite Knowledge and Fragmented Memento of Epoch Challenges. Before, such an item went into the bags, where its use effect could be cast again and again. Blizzard Watch describes threads as consumed automatically when looted; the tooltips say a memento is absorbed automatically and Bronze Cluster grants Bronze when looted. The server loads 339 items with this trigger. Consumables whose only effect is the looted one are applied the same way, 96 more, so 151 are applied and 188 are stored as before; some of those are opened or used from the bags, such as Delver's Starter Kit and the Mists "A Little Patience" supplies. Players and Blizzard's own tooltips describe most of the 96 being used up on loot: the Dragon Isles supply herbs, companion experience, Kaja'Cola drinks, mysterious potions and delve boons. The four delve blessings, Sanctified Touch and Glimmering Crystal have no such evidence, and the Abyss Anglers upgrades are bought rather than looted. For many of the 96 the looted spell does nothing here yet (the Dragon Isles herbs have no spell loot and grant no supplies, and the boons, most drinks and the blessings need spell scripts), and none of them drops on this server today.
