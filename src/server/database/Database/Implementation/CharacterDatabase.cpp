...
#define CharacterSelectEquipment(table_alias) \
    ...
    PrepareStatement(CHAR_UPD_BATTLEPAY_DISTRIBUTION_RETURN, "UPDATE battlepay_account_distribution SET status = ?, consumed = 0, applied = 0, targetCharacter = 0, specId = 0 WHERE distributionId = ? AND accountId = ?", CONNECTION_BOTH);
    PrepareStatement(CHAR_UPD_BATTLEPAY_DISTRIBUTION_RETURN_BY_CHAR, "UPDATE battlepay_account_distribution SET status = 1, consumed = 0, applied = 0, targetCharacter = 0, specId = 0 WHERE targetCharacter = ? AND accountId = ? AND consumed = 1 AND applied = 0", CONNECTION_BOTH);
    PrepareStatement(CHAR_DEL_BATTLEPAY_DISTRIBUTION, "DELETE FROM battlepay_account_distribution WHERE distributionId = ? AND accountId = ?", CONNECTION_BOTH);
    PrepareStatement(CHAR_DEL_BATTLEPAY_DISTRIBUTION_APPLIED_BY_CHAR, "DELETE FROM battlepay_account_distribution WHERE targetCharacter = ? AND accountId = ? AND applied = 1", CONNECTION_BOTH);
